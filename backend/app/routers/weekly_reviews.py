...
async def _build_review(db: AsyncSession, user_id: int, week_start: date, week_end: date) -> WeeklyReview:
    """Compute stats and upsert a WeeklyReview row."""
    ...
    agg = await db.execute(
        select(
            ...
            func.sum(DailyCheckin.old_script_triggered.cast(db.bind.dialect.NUMERIC)).label("triggered_days"),
            func.sum(DailyCheckin.old_script_resisted.cast(db.bind.dialect.NUMERIC)).label("resisted_days"),
        ).where(
    ...
    trigger_count = (await db.execute(
    ...
    experiments_done = (await db.execute(
