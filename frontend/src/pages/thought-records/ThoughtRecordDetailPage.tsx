...
export default function ThoughtRecordDetailPage() {
  ...
  // Support both field naming conventions (API snake_case vs wizard-local _text suffix)
  const FIELDS = [
    { label: 'Ситуация',              value: record.situation        ?? record.situation_text },
    { label: 'Автоматическая мысль',  value: record.auto_thought     ?? record.automatic_thought_text },
    { label: 'Значение',              value: record.meaning          ?? record.meaning_text },
    { label: 'Страх',                 value: record.fear_text },
    { label: 'Старый закон',          value: record.old_law          ?? record.old_law_text },
    { label: 'Реакция тела',          value: record.body_response    ?? record.body_reaction_text },
    { label: 'Действие',              value: record.action_taken_text },
    { label: 'Доказательства «за»',   value: record.evidence_for     ?? record.evidence_for_text },
    { label: 'Доказательства «против»', value: record.evidence_against ?? record.evidence_against_text },
    { label: 'Игнорируемые факты',    value: record.ignored_facts_text },
    { label: 'Сбалансированная мысль', value: record.new_perspective  ?? record.balanced_thought_text },
    { label: 'Новое действие',        value: record.new_action       ?? record.new_action_text },
  ];
  ...
  return (
    <div className="detail-page">
      <div className="detail-header">
        ...
        <button
          onClick={() => { if (confirm('Удалить эту запись?')) deleteMutation.mutate(); }}
      ...
      {(record.emotion_before_score || record.emotion_after_score) && (
        <div className="emotion-scores">
          <div className="score-item"><span className="score-label">До</span><span className="score-value">{record.emotion_before_score}/10</span></div>
          <div className="score-arrow">→</div>
          <div className="score-item"><span className="score-label">После</span><span className="score-value">{record.emotion_after_score}/10</span></div>
        </div>
      )}
      ...
      {distortionNames.length > 0 && (
        <div className="detail-section">
          ...
          <div className="distortions-list">
            {distortionNames.map(name => <span key={name} className="tag">{name}</span>)}
          </div>
        </div>
      )}

      {(record.linked_trigger_id ?? record.trigger_event_id) && (
        <div className="detail-section">
          <Link
            to={`/triggers/${record.linked_trigger_id ?? record.trigger_event_id}`}
            className="btn btn-ghost btn-sm"
          >Связанный триггер →</Link>
        </div>
      )}
    </div>
  );
